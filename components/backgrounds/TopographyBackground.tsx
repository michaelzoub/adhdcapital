...
export interface TopographyBackgroundProps {
  ...
  /** Line thickness */
  strokeWidth?: number;
  /** Use fixed positioning (full viewport) instead of filling the positioned parent */
  fixed?: boolean;
}
...
export function TopographyBackground({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const safeLineCount = Math.max(2, lineCount);
      const spacing = height / (safeLineCount - 1);
      const padding = 50;
